...
| P0-MY-6 | □ cleared seed | Commuter | `KTM-Komuter-Tanjung-Malim-Port-Klang` | KTM Komuter Tanjung Malim - Port Klang | Tanjung Malim ⇄ Pelabuhan Klang（131 km） | 下一輪可補 full-corridor 站表、KTM EMU template、OSM relation、KTM Komuter icon | 同上；採 official `Pelabuhan Klang` label，script-side aliases 可含 `Port Klang` |
...
2. 泰國前 2 條 monorail seed、`BKK-SRT-Dark-Red`、`BKK-SRT-Light-Red`、`KL-Monorail`、`SG-LRT-Bukit-Panjang`、`KL-LRT-Ampang`、`KL-LRT-Sri-Petaling`、`ERL-KLIA-Transit`、`SG-LRT-Sengkang`、`SG-LRT-Punggol`、`ERL-KLIA-Ekspres` 與 `KTM-Komuter-Batu-Caves-Pulau-Sebang` 已完成；下一個 5.3 seed 可接 `KTM-Komuter-Tanjung-Malim-Port-Klang`，之後再做其他 cleared seeds。Dark / Light Red 完成後不批量補完整曼谷，改依 P0 放行狀態在 Malaysia / Singapore cleared seeds 間輪替。
...
5. `ERL-KLIA-Transit` all-stop local airport service、`ERL-KLIA-Ekspres` regular-hours 3-station seed、`KTM-Komuter-Batu-Caves-Pulau-Sebang` full-corridor seed 已完成；下一條 KTM Komuter Klang Valley full-corridor seed 可接 `KTM-Komuter-Tanjung-Malim-Port-Klang`。
...
6. 已完成 Malaysia airport/commuter gate 的 cleared seeds：`ERL-KLIA-Transit`、`ERL-KLIA-Ekspres` regular-hours express 與 `KTM-Komuter-Batu-Caves-Pulau-Sebang` full-corridor seed；KTM Komuter 長線 short-turn policy 已放行為 full-corridor seed，非 canonical service-pattern runtime 仍另案。
...
5. [ ] 持續追加入列 backlog 的候選新線前：先完成「單線 seed」→ 生成 icon/template → 驗證 → 推進同營運者其餘線路；`BKK-MRT-Yellow`、`BKK-MRT-Pink` 主線、`BKK-SRT-Dark-Red`、`BKK-SRT-Light-Red`、`KL-Monorail`、`SG-LRT-Bukit-Panjang`、`KL-LRT-Ampang`、`KL-LRT-Sri-Petaling`、`ERL-KLIA-Transit`、`SG-LRT-Sengkang`、`SG-LRT-Punggol`、`ERL-KLIA-Ekspres` 與 `KTM-Komuter-Batu-Caves-Pulau-Sebang` 已完成，下一個 5.3 seed 可接 `KTM-Komuter-Tanjung-Malim-Port-Klang`。
...
- `scope`: Asia P0 seed order and handoff policy. `BKK-SRT-Dark-Red`, `BKK-SRT-Light-Red`, `KL-Monorail`, `SG-LRT-Bukit-Panjang`, `KL-LRT-Ampang`, `KL-LRT-Sri-Petaling`, `ERL-KLIA-Transit`, `SG-LRT-Sengkang`, `SG-LRT-Punggol`, `ERL-KLIA-Ekspres`, and `KTM-Komuter-Batu-Caves-Pulau-Sebang` were completed in 2026-05-15 5.3 rounds; the current 5.3 next target can move to `KTM-Komuter-Tanjung-Malim-Port-Klang` before additional optional / monitor items.
...
- `report`: 新增/修改 region 0、line 0、station 0、train template 0、shape mapping 0、icon 0；完成 1 個 5.5 seed cadence 決策。`BKK-SRT-Dark-Red`、`BKK-SRT-Light-Red`、`KL-Monorail`、`SG-LRT-Bukit-Panjang`、`KL-LRT-Ampang`、`KL-LRT-Sri-Petaling`、`ERL-KLIA-Transit`、`SG-LRT-Sengkang`、`SG-LRT-Punggol`、`ERL-KLIA-Ekspres` 與 `KTM-Komuter-Batu-Caves-Pulau-Sebang` 已於 2026-05-15 5.3 seed 完成；下一個可下放 seed 轉 `KTM-Komuter-Tanjung-Malim-Port-Klang`。
...
- `report`: 新增/修改 region 0、line 0、station 0、train template 0、shape mapping 0、icon 0；完成 1 個 5.5 KTM Komuter full-corridor gate。`KTM-Komuter-Batu-Caves-Pulau-Sebang` 已於後續 5.3 seed 完成；`KTM-Komuter-Tanjung-Malim-Port-Klang` 仍可接續排程。blocked remains：non-canonical short-turn/partial timetable service-pattern runtime and RTS Link passenger-service monitor.
