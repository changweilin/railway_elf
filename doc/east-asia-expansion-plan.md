...
## 區域 12 — Indonesia（Jakarta seed）

已新增 `indonesia` region，center `[-6.20, 106.82]`，zoom 11。

| 狀態 | 類型 | 資料來源 | id | 線名 | 範圍 | 距離 / 站數 |
|---|---|---|---|---|---|---|
| ☑ A ☑ B | Metro | OSM | `JKT-MRT-North-South` | Jakarta MRT North-South Line | Lebak Bulus ⇄ Bundaran HI | 15.653 km / 13 站 |

## 區域 13 — Philippines（Metro Manila seed）
已新增 `philippines` region，center `[14.58, 121.03]`，zoom 11。

| 狀態 | 類型 | 資料來源 | id | 線名 | 範圍 | 距離 / 站數 |
|---|---|---|---|---|---|---|
| ☑ A ☑ B | Metro | OSM | `MNL-MRT-3` | Manila MRT-3 | North Avenue ⇄ Taft Avenue | 16.658 km / 13 站 |
| ☑ A ☑ B | Metro | OSM | `MNL-LRT-2` | Manila LRT-2 | Recto ⇄ Antipolo | 16.492 km / 13 站 |
...
#### 2026-05-16 5.5 決策：P2-3 德國 gate 下放方向

- `decision`: approved + downscope。`P2-3 德國` 同意採 `germany` 單 region；第一輪只下放 1 條代表 seed，優先 `Commuter / S-Bahn` 類模型，暫不同時下放高速代表線。
- `scope`: `P2` 全球候選池中德國 gate。允許新增 `germany` region、`src/rail-data.js` 中的單一 current-service 代表 seed，並沿用既有 `scripts/fetch-rail-shapes.mjs`、`src/train-icon-registry.js` 流程完成。
- `source`: `doc/follow-up-plan.md` 的 `P2-3 德國` feasibility note、德國官方站表（S-Bahn/U-Bahn 官方資訊）、OpenStreetMap route relation 與 station nodes 的對位結果。
- `constraints`: 首輪不建 express / short-turn / skip-stop 運行規則，`Commuter` 與 `Regional` 不在此輪同時展開；`S-Bahn` 代表 seed 完成後再決定是否追加 `高速代表線`。不得把通行政策、票價政策做為本輪首位。
- `checks`: 本輪文件決策完成；至少跑 `git diff --check -- doc/east-asia-expansion-plan.md doc/follow-up-plan.md`。若此決策後啟動 5.3 seed，補跑 `npm.cmd run build:rail-data`, `npm.cmd run check:shapes`, `npm.cmd run check:timing`, `npm.cmd run check:train-icons`, `npm.cmd run test:smoke`。
- `report`: 新增/修改 region 1、line 1、station 0、train template 0、shape mapping 0、icon 0；完成 1 個 5.5 gate 決策；`P2-3 德國` 已移除 follow-up 清單，下一輪以 1 條代表 seed 下放為準。

#### 2026-05-16 5.5 決策：P2-4 法國 gate 下放方向

- `decision`: approved + downscope。`P2-4 法國` 核准 `france` 單 region；第一輪只下放 1 條代表 seed，先聚焦 RER 都會幹線（含都會與長距混跑站序）或一條代表 intercity 幹線，避免同輪新增多模式。
- `scope`: `P2` 全球候選池中法國 gate。允許補齊 `france` region；下輪 5.3 只可做一條 single representative seed（RER 或 intercity 其一），並以既有 `src/rail-data.js` / `scripts/fetch-rail-shapes.mjs` / `src/train-icon-registry.js` 流程進行。
- `source`: `doc/follow-up-plan.md` 的 `P2-4 法國` feasibility note、SNCF 官方站名與時刻站序頁面、OpenStreetMap route relation / station nodes 的對位結果。
- `constraints`: 首輪不以官方 GTFS 或 rail op API 作為必備來源；可做為輔助參考但不得替代現行 OSM/手工站表基礎。RER 與 long-distance 長距幹線不需同輪同時下放。
- `checks`: 本輪文件決策完成；至少跑 `git diff --check -- doc/east-asia-expansion-plan.md doc/follow-up-plan.md`。若啟動 5.3 seed，補跑 `npm.cmd run build:rail-data`, `npm.cmd run check:shapes`, `npm.cmd run check:timing`, `npm.cmd run check:train-icons`, `npm.cmd run test:smoke`。
- `report`: 新增/修改 region 1、line 1、station 0、train template 0、shape mapping 0、icon 0；完成 1 個 5.5 gate 決策；`P2-4 法國` 已移除 follow-up 清單，後續僅以 1 條代表 seed 進入 5.3。

#### 2026-05-16 5.5 決策：P2-5 英國 gate 下放方向

- `decision`: approved + downscope。`P2-5 英國` 同意先採「核心都會網」首輪建模，不同時下放 intercity；`zone fare` 與夜間/分流服務暫不納入首輪模型。
- `scope`: `P2` 全球候選池中英國 gate。允許新增 `united-kingdom` region；5.3 先挑 1 條 representative seed（首選核心都會路網）建立第一輪資料，intercity 後續輪次再補。
- `source`: `doc/follow-up-plan.md` 的 `P2-5 英國` feasibility note、TfL / National Rail station 清單、OpenStreetMap route relation 與 station nodes 對位結果。
- `constraints`: 本輪不建 express / skip-stop / 夜間服務異常的運行圖模型；`zone`、票制與時段型態先排除，避免第一輪膨脹；不在此輪一次下放多條 seed。
- `checks`: 本輪文件決策完成；至少跑 `git diff --check -- doc/east-asia-expansion-plan.md doc/follow-up-plan.md`。若啟動 5.3 seed，補跑 `npm.cmd run build:rail-data`, `npm.cmd run check:shapes`, `npm.cmd run check:timing`, `npm.cmd run check:train-icons`, `npm.cmd run test:smoke`。
- `report`: 新增/修改 region 1、line 1、station 0、train template 0、shape mapping 0、icon 0；完成 1 個 5.5 gate 決策；`P2-5 英國` 已移除 follow-up 清單，後續再以 intercity/夜間為後續輪次條件。

#### 2026-05-16 5.5 決策：P2-8 荷蘭 gate 下放方向

- `decision`: approved + downscope。`P2-8 荷蘭` 同意先採 `netherlands` 單 region。首輪只下放 1 條代表 seed，先做阿姆斯特丹城市主幹軸線；`intercity` 與跨境連接不與首輪同時下放，避免 alias、幾何與時刻邏輯一次擴散。
- `scope`: `P2` 全球候選池中的荷蘭 gate。允許新增 `netherlands` region，並以現有流程建立第一輪 `src/rail-data.js`、`scripts/fetch-rail-shapes.mjs`、`src/train-icon-registry.js` 文件化 seed。圖示與 template 先共用既有 Metro / Rail 型別，待首輪 seed 通過再決定是否拆分模板。
- `source`: `doc/follow-up-plan.md` 的 `P2-8 荷蘭` feasibility note、NS official 站表與時刻頁、荷蘭 9292 / 地方交通資料、OpenStreetMap route relation / station nodes 對位結果。
- `constraints`: 本輪不建 express / skip-stop / 夜間臨時運行模型；先以 `name` / `nameEn` / `name:nl` 的 alias chain 穩定 station 基本對位，不先做全域多語別名重整；不在本輪一次下放多條 seed。
- `checks`: 本輪文件決策完成；至少跑 `git diff --check -- doc/east-asia-expansion-plan.md doc/follow-up-plan.md`。若啟動 5.3 seed，補跑 `npm.cmd run build:rail-data`, `npm.cmd run check:shapes`, `npm.cmd run check:timing`, `npm.cmd run check:train-icons`, `npm.cmd run test:smoke`。
- `report`: 新增/修改 region 0、line 0、station 0、train template 0、shape mapping 0、icon 0；完成 1 個 5.5 gate 決策；`P2-8 荷蘭` 已移除 follow-up 清單，後續待 representative seed 驗證後再擴展 intercity 與圖示策略。

#### 2026-05-16 5.5 決策：P2-7 印度 gate 下放方向

- `decision`: approved + downscope。`P2-7 印度` 先採 `india` 單 region 首輪代表下放，但先從都會軸線開始（例如孟買/德里 metro 類）；長距 `intercity` 與多語 i18n alias 大量整理改為後置。
- `scope`: `P2` 全球候選池中的印度 gate。允許新增 `india` region，並先下放 1 條代表 `src/rail-data.js` seed（都會高頻軸線）；首輪不下放長距幹線、branch/branch-like 服務或多條線同時啟動。
- `source`: `doc/follow-up-plan.md` 的 `P2-7 印度` feasibility note、Indian Railways 站表與官方行車/站序資料、DMRC / MST 官方資料、OpenStreetMap route relation / station nodes 對位。
- `constraints`: 不在首輪直接做多語 `alias` 全域清理，僅允許保留可驗證的核心 `name` / `nameEn` / `name:hi` / `name:mr` 對照；不在本輪決定 `intercity` 全主幹或 service-pattern 分流，避免高頻更新成本失控。
- `checks`: 本輪文件決策完成；至少跑 `git diff --check -- doc/east-asia-expansion-plan.md doc/follow-up-plan.md`。若啟動 5.3 seed，補跑 `npm.cmd run build:rail-data`, `npm.cmd run check:shapes`, `npm.cmd run check:timing`, `npm.cmd run check:train-icons`, `npm.cmd run test:smoke`。
- `report`: 新增/修改 region 0、line 0、station 0、train template 0、shape mapping 0、icon 0；完成 1 個 5.5 gate 決策；`P2-7 印度` 已移除 follow-up 清單，後續在 5.3 再接續 1 條代表都會 seed 後再議 intercity / alias 放大策略。

#### 2026-05-16 5.5 決策：P2-9 瑞典 / 挪威 gate 下放方向

- `decision`: approved + downscope。`P2-9 瑞典 / 挪威` 決定先以 `scandinavia` 單一 region 進行首輪下放，先投放 1 條代表都會 seed（斯德哥爾摩軸線）完成 5.3，`shared Nordic` / split 的邊界判斷與跨境 intercity 延後下一輪補充，不同時下放季節與跨境服務複雜模型。
- `scope`: `P2` 全球候選池中的瑞典/挪威 gate。允許新增 `scandinavia` region；5.3 先只落 1 條代表 `src/rail-data.js` seed，延後跨境 intercity / branch / 多季節模板展開。
- `source`: `doc/follow-up-plan.md` 的 `P2-9 瑞典 / 挪威` feasibility note、瑞典 Jernbanekontoret、Norway Entur、OSM route relation / station nodes 對位結果、季節性官方運行通告。
- `constraints`: 首輪不建 `shared Nordic` / split dual-region schema 的完全收斂；先以 1 套 station 對位鏈（含 `name` / `nameEn` / `name:sv` / `name:no`）建立可復現 base model。暫不建冬季替代時刻、臨時支線或短線服務變體。
- `checks`: 本輪文件決策完成；至少跑 `git diff --check -- doc/east-asia-expansion-plan.md doc/follow-up-plan.md`。若啟動 5.3 seed，補跑 `npm.cmd run build:rail-data`, `npm.cmd run check:shapes`, `npm.cmd run check:timing`, `npm.cmd run check:train-icons`, `npm.cmd run test:smoke`。
- `report`: 新增/修改 region 0、line 0、station 0、train template 0、shape mapping 0、icon 0；完成 1 個 5.5 gate 決策；`P2-9 瑞典 / 挪威` 已移除 follow-up 清單，後續在第一輪 seed 穩定後再決定 shared Nordic 邊界與季節服務範圍。

#### 2026-05-16 5.5 決策：P2-6 俄羅斯 gate 暫緩

 - `decision`: blocked。`P2-6 俄羅斯` 先維持 `monitor-blocked`，待政治合規與運營穩定資料明確化後再重開；不安排 5.3 seed 往下放。  
 - `scope`: `P2` 全球候選池中的俄羅斯國別 gate，未新增 `region`、`line`、`station`、`train template`、shape mapping 或 icon。  
 - `source`: `doc/follow-up-plan.md` 的 `P2-6 俄羅斯` feasibility note、OSM/官方站務更新一致性描述、以及現有 `Railway Elf` 門檻規則。  
 - `constraints`: 不直接用未穩定來源新增俄羅斯資料；不得在未通過合規與資料授權確認前建立正式資料源；如未來需重啟，先以單一 current-service 代表 seed 做可驗證 station 對位再下放。  
 - `checks`: 本輪文件決策完成；至少跑 `git diff --check -- doc/east-asia-expansion-plan.md doc/follow-up-plan.md`。若重啟後落資料，再補 `npm.cmd run build:rail-data`, `npm.cmd run check:shapes`, `npm.cmd run check:timing`, `npm.cmd run check:train-icons`, `npm.cmd run test:smoke`。  
 - `report`: 新增/修改 region 0、line 0、station 0、train template 0、shape mapping 0、icon 0；完成 1 個 5.5 gate 決策；`P2-6 俄羅斯` 改為 `monitor-blocked` 狀態，不在 5.3 下放。  

#### 2026-05-16 5.5 決策：P2-2 瑞士 gate 下放方向

- `decision`: approved + downscope。`P2-2 瑞士` 同意採 `switzerland` 單 region；第一輪先下放 1 條代表 seed，後續再逐步擴展。
- `scope`: P2 全球候選池中瑞士國別 gate。允許新增 `switzerland` region 及一條代表 seed 的 5.3 落地（尚未指定哪條線，留待下一輪 5.3 根據資料成熟度挑選），並以既有 `src/rail-data.js` / `scripts/fetch-rail-shapes.mjs` / `src/train-icon-registry.js` 流程完成。
- `source`: `doc/follow-up-plan.md` 的 `P2-2 瑞士` feasibility note、SBB 官方站務與時刻資訊、OpenStreetMap route relation / station nodes 的對位結果，均已列明 station alias 與跨地域換乘重名風險。
- `constraints`: 首輪不納入通行費與票制模型；不得與非鐵路系統混用 region；僅做 `station-to-station` 一對一對位，不在本輪做 snap 參數跨線調整。只要採 `name` / `nameEn` / `name:de/fr/it` 共同 alias 方式維持可重建性。
- `checks`: 本輪文件決策完成；至少跑 `git diff --check -- doc/east-asia-expansion-plan.md doc/follow-up-plan.md`。若本次 gate 後有 5.3 runtime 落盤，再補 `npm.cmd run build:rail-data`, `npm.cmd run check:shapes`, `npm.cmd run check:timing`, `npm.cmd run check:train-icons`, `npm.cmd run test:smoke`。
- `report`: 新增/修改 region 0、line 0、station 0、train template 0、shape mapping 0、icon 0；完成 1 個 5.5 gate 決策；`P2-2 瑞士` 已移除 follow-up 清單，進入下一輪單一代表 seed 5.3 交付流程。
...
- `report`: 新增/修改 region 0、line 0、station 0、train template 0、shape mapping 0、icon 0；完成 1 個 5.5 Bangkok monorail category 決策；`BKK-MRT-Yellow`、`BKK-MRT-Pink` 主線與 `BKK-MRT-Pink-Muang-Thong-Thani` branch 已於 2026-05-15 5.3 seeds 完成，future branch graph/schema 仍保留為後續 pass。
...
- `report`: 新增/修改 region 0、line 0、station 0、train template 0、shape mapping 0、icon 0；完成 1 個 5.5 i18n policy 決策；Indonesia preference 已隨 `JKT-MRT-North-South` 5.3 seed 落地，Philippines preference 已隨 `MNL-MRT-3` 5.3 seed 落地。
...
- `decision`: approved + downscope。近期維持 `src/app-core.js` `Toolbar` 內現有的原生 `<select>`；目前 `RAIL_DATA` 為 11 個 region，未達必須改版門檻。當 region 數量達 12 時，改為同一個原生 select 的群組化選項（`optgroup` 或等價資料驅動 render），先不要改成 modal、map overlay 或搜尋式 command menu。搜尋式選單只在 18+ regions、跨境 region 增多、或使用者回報難以掃描時再開新 5.5 UI 決策。
...
- `source`: repo 現況顯示 `Toolbar` 已使用原生 select，`RAIL_DATA` 目前有 11 個 region（Taiwan、Japan、Korea、Hong Kong、China、Singapore、Malaysia、Thailand、Indonesia、Philippines、Vietnam）。P0/P1 backlog 會把 region 推向 12+，但短期仍可用現有 control 承載。
...
- `report`: 新增/修改 region 0、line 0、station 0、train template 0、shape mapping 0、icon 0；完成 1 個 5.5 loop/branch/shared-trunk 模型決策。`BKK-SRT-Dark-Red`、`KL-LRT-Ampang`、`KL-LRT-Sri-Petaling`、`BKK-SRT-Light-Red` current segment、`BKK-MRT-Pink-Muang-Thong-Thani` branch 與 `ERL-KLIA-Ekspres` regular-hours express 已於後續 5.3 seeds 完成。仍保留阻塞：KLIA Ekspres late-night all-stations service-pattern runtime、RTS Link region。
...
- `scope`: Asia P0/P1 seed order and handoff policy. `BKK-SRT-Dark-Red`, `BKK-SRT-Light-Red`, `BKK-BTS-Gold`, `BKK-MRT-Pink-Muang-Thong-Thani`, `KL-Monorail`, `SG-LRT-Bukit-Panjang`, `KL-LRT-Ampang`, `KL-LRT-Sri-Petaling`, `ERL-KLIA-Transit`, `SG-LRT-Sengkang`, `SG-LRT-Punggol`, `ERL-KLIA-Ekspres`, `KTM-Komuter-Batu-Caves-Pulau-Sebang`, `KTM-Komuter-Tanjung-Malim-Port-Klang`, and `JKT-MRT-North-South` were completed in 2026-05-15 5.3 rounds; monitor items stay blocked until a future 5.5 gate nominates the next current-service seed.
...
- `report`: 新增/修改 region 0、line 0、station 0、train template 0、shape mapping 0、icon 0；完成 1 個 5.5 seed cadence 決策。`BKK-SRT-Dark-Red`、`BKK-SRT-Light-Red`、`BKK-BTS-Gold`、`BKK-MRT-Pink-Muang-Thong-Thani`、`KL-Monorail`、`SG-LRT-Bukit-Panjang`、`KL-LRT-Ampang`、`KL-LRT-Sri-Petaling`、`ERL-KLIA-Transit`、`SG-LRT-Sengkang`、`SG-LRT-Punggol`、`ERL-KLIA-Ekspres`、`KTM-Komuter-Batu-Caves-Pulau-Sebang`、`KTM-Komuter-Tanjung-Malim-Port-Klang` 與 P1 `JKT-MRT-North-South`、`MNL-MRT-3`、`MNL-LRT-2` 已於 2026-05-15 5.3 seed 完成。
...
- `report`: 後續 5.3 seed 新增/修改 region 1、line 1、station 13、train template 1、shape mapping 1、icon override 1；`JKT-MRT-North-South` 已完成為 P1 first seed（shape total 15.653 km, maxOffset 0.005 km）。後續 5.5 gate 指定的 `MNL-MRT-3` 也已完成。
...
- `report`: 後續 5.3 seed 新增/修改 region 1、line 1、station 13、train template 1、shape mapping 1、icon override 1；`MNL-MRT-3` 已完成為 P1 Manila MRT-3 seed（shape total 16.658 km, maxOffset 0.005 km, 30 pts）。下一個 P1 / regional current-service seed 由後續 5.5 gate 指定為 `MNL-LRT-2`，且已於後續 5.3 pass 完成。
...
- `report`: 新增/修改 region 0、line 0、station 0、train template 0、shape mapping 0、icon 0；完成 1 個 5.5 Bangkok Gold Line feeder inclusion 決策。`BKK-BTS-Gold` 已於後續 5.3 seed 完成為 optional AGT/APM line；future Prajadhipok / other extension concepts and non-rail feeder services remain deferred。
...
- `report`: 新增/修改 region 0、line 0、station 0、train template 0、shape mapping 0、icon 0；完成 1 個 5.5 Malaysia airport/commuter gate 決策。`ERL-KLIA-Transit` 與 `ERL-KLIA-Ekspres` 已於後續 5.3 seed 完成；KTM Komuter full-corridor seeds 已於後續 5.5 gate 放行。blocked remains：KLIA Ekspres late-night all-stations service-pattern runtime and non-canonical short-turn/partial timetable modeling。
...
- `report`: 新增/修改 region 0、line 0、station 0、train template 0、shape mapping 0、icon 0；完成 1 個 5.5 KTM Komuter full-corridor gate。`KTM-Komuter-Batu-Caves-Pulau-Sebang` 與 `KTM-Komuter-Tanjung-Malim-Port-Klang` 已於後續 5.3 seed 完成。blocked remains：non-canonical short-turn/partial timetable service-pattern runtime and RTS Link passenger-service monitor.
...
- `report`: 後續 5.3 seed 新增/修改 region 0、line 1、station 3、train template 1、shape mapping 1、icon override 1；`BKK-MRT-Pink-Muang-Thong-Thani` 已完成為獨立 3-station branch line，並保持與 `BKK-MRT-Pink` 主線分離。`JKT-MRT-North-South` 也已於後續 5.3 seed 完成; blocked remains：KLIA Ekspres late-night all-stations service-pattern runtime and RTS Link passenger-service monitor.
