...
5. [ ] 持續追加入列 backlog 的候選新線前：先完成「單線 seed」→ 生成 icon/template → 驗證 → 推進同營運者其餘線路；`BKK-MRT-Yellow`、`BKK-MRT-Pink` 主線已完成，`BKK-SRT-Dark-Red` 已由 5.5 放行為獨立 Red Line seed，若泰國 queue 暫停則接 `KL-Monorail`。
