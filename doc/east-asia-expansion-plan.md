...
| ◐ seed | LRT / AGT | 輕量捷運與無人自動運轉線 | Gimpo Goldline（☑ A ☑ B）、Incheon 2（☑ A ☑ B）、Ui LRT（☑ A ☑ B）；Sillim Line、Uijeongbu U Line、Yongin EverLine、Busan-Gimhae LRT 等待後續 | OSM + operator data | Gimpo Goldline、Incheon 2 與 Ui LRT 已作為 low-capacity / driverless seeds；Ui LRT 圖示先檢查 Gimpo / Incheon 既有 LRT，但 Hyundai Rotem UL000 與 연둣빛塗裝不同而新增 line-aware 圖示；後續可沿用或擴充此圖示風格 |
...
| LRT | 3 | Gimpo Goldline、Incheon 2、Ui LRT（台灣側已涵蓋淡海、高雄環狀輕軌） |
