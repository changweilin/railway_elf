...
- **2026-05-15 P1 update** — `JKT-MRT-North-South` 已完成為 Indonesia 的第一條 current-service seed：13 站、OSM relation `9677669`、generated shape 15.653 km、maxOffset 0.005 km。歷史 34/34 批次數字保留為原 East Asia baseline；新增 Jakarta seed 另列於區域 12。
...
| P1-PH | 菲律賓 | Manila LRT-1、LRT-2、MRT-3、PNR / NSCR future corridor | `MNL-MRT-3` 可作下一條 current-service seed（North Avenue ⇄ Taft Avenue，13 站） | LRT-1 extension、LRT-2 west extension、PNR / NSCR future corridor 需確認 current baseline；KAI/PNR 類長線不併入本 seed |
...
5. P1 第一條 seed 已完成：印尼 `JKT-MRT-North-South`（Jakarta MRT North-South Phase 1，Lebak Bulus ⇄ Bundaran HI）；下一條 current-service seed 指定為菲律賓 `MNL-MRT-3`（North Avenue ⇄ Taft Avenue，13 站）。多 region UI 已決定維持原生 select，12+ regions 時改為群組化 select。
...
14. 已決定 P1 下一條 current-service seed：`MNL-MRT-3` Manila MRT-3，先建 North Avenue ⇄ Taft Avenue 13-station current line；LRT-1 extension、LRT-2 west extension、PNR / NSCR future corridor 不併入本 seed。
...
- **多 region 切換 UI**：地區數從 2 → 10，要看 `src/app-core.js` 的 region selector 是否仍適合（例如改下拉式或分組）。
...
18. [x] 指定 P1 下一條 current-service seed：`MNL-MRT-3` Manila MRT-3（North Avenue ⇄ Taft Avenue，13 站）可下放 5.3；LRT-1 extension、LRT-2 west extension、PNR / NSCR future corridor 仍另案。
...
- `source`: repo 現況已覆蓋 Taiwan/Japan/Korea/Hong Kong/China/Singapore/Malaysia/Thailand/Indonesia/Vietnam 的 Nominatim language preferences and OSM `name:<lang>` priority. Current data uses local scripts for `name`, English/Latin for most `nameEn`, and region-specific direction labels.
...
- `report`: 新增/修改 region 0、line 0、station 0、train template 0、shape mapping 0、icon 0；完成 1 個 5.5 i18n policy 決策；Indonesia preference 已隨 `JKT-MRT-North-South` 5.3 seed 落地，Philippines region preference 留待 future seed。
...
- `decision`: approved + downscope。近期維持 `src/app-core.js` `Toolbar` 內現有的原生 `<select>`；目前 `RAIL_DATA` 為 10 個 region，未達必須改版門檻。當 region 數量達 12 時，改為同一個原生 select 的群組化選項（`optgroup` 或等價資料驅動 render），先不要改成 modal、map overlay 或搜尋式 command menu。搜尋式選單只在 18+ regions、跨境 region 增多、或使用者回報難以掃描時再開新 5.5 UI 決策。
...
- `source`: repo 現況顯示 `Toolbar` 已使用原生 select，`RAIL_DATA` 目前有 10 個 region（Taiwan、Japan、Korea、Hong Kong、China、Singapore、Malaysia、Thailand、Indonesia、Vietnam）。P0/P1 backlog 會把 region 推向 12+，但短期仍可用現有 control 承載。
...
- `report`: 後續 5.3 seed 新增/修改 region 1、line 1、station 13、train template 1、shape mapping 1、icon override 1；`JKT-MRT-North-South` 已完成為 P1 first seed（shape total 15.653 km, maxOffset 0.005 km）。下一個 P1 / regional current-service seed 由後續 5.5 gate 指定為 `MNL-MRT-3`。

#### 2026-05-15 5.5 決策：P1 Manila MRT-3 next seed

- `decision`: approved + downscope。`JKT-MRT-North-South` 完成後，下一條 P1 current-service seed 指定為 `MNL-MRT-3` Manila MRT-3。5.3 只建現行 North Avenue ⇄ Taft Avenue 13-station line、新增 `philippines` region、MRT-3 train template/icon、OSM relation/shape 與 Philippines name-tag preference；不把 LRT-1、LRT-2、PNR / NSCR 或任何 future extension 併入同一輪。
- `scope`: Manila MRT-3 current EDSA corridor only. Future Spark-owned files are `src/rail-data.js`, `scripts/fetch-rail-shapes.mjs`, `src/train-icon-registry.js`, train icon scripts/assets, generated Philippines shape outputs, `src/app-core.js` region selector only if needed for the new region, and narrow docs.
...
- `checks`: For this policy-only docs pass, run `git diff --check -- doc/east-asia-expansion-plan.md doc/follow-up-plan.md doc/railway-elf-sop.md`. For the future MRT-3 data seed, run `npm.cmd run build:train-icons`, `node --env-file-if-exists=.env scripts/fetch-rail-shapes.mjs --only-lines=MNL-MRT-3`, `npm.cmd run build:rail-data`, `npm.cmd run check:train-icons`, `npm.cmd run check:timing`, `npm.cmd run check:shapes`, `npm.cmd run build`, and `npm.cmd run test:smoke`.
- `report`: 新增/修改 region 0、line 0、station 0、train template 0、shape mapping 0、icon 0；完成 1 個 5.5 P1 Manila MRT-3 next-seed decision。下一個可下放 5.3 seed 是 `MNL-MRT-3`。
