...
function mergeShapes(shapes, regionKey) {
  ...
  for (const [, region] of regions) {
    ...
    for (const line of region.lines) {
      ...
      if (gen.stationCoords || gen.stationCoordsByIndex) {
        const generatedStationCoordsInOrder = Array.isArray(gen.stationCoordsByIndex) &&
          gen.stationCoordsByIndex.length === line.stations.length
            ? gen.stationCoordsByIndex
            : (gen.stationCoords && Object.keys(gen.stationCoords).length === line.stations.length
              ? Object.values(gen.stationCoords)
              : null);
        const generatedStationNamesInOrder = gen.stationKms ? Object.keys(gen.stationKms) : [];
        for (let i = 0; i < line.stations.length; i++) {
          const st = line.stations[i];
          const coord = generatedStationCoordsInOrder?.[i] || gen.stationCoords?.[st.name];
          if (!coord) continue;
          st.lat = coord[0];
          st.lng = coord[1];
        }
        if (gen.stationCoords && !generatedStationCoordsInOrder && generatedStationNamesInOrder.length === line.stations.length) {
          for (const [name, coord] of Object.entries(gen.stationCoords)) {
            const idx = generatedStationNamesInOrder.indexOf(name);
            if (idx < 0 || idx >= line.stations.length) continue;
            line.stations[idx].lat = coord[0];
            line.stations[idx].lng = coord[1];
          }
        }
      }

      let useGenStationKms = true;
      const generatedStationKmsInOrder = Array.isArray(gen.stationKmsByIndex) &&
        gen.stationKmsByIndex.length === line.stations.length
          ? gen.stationKmsByIndex
          : (gen.stationKms && Object.keys(gen.stationKms).length === line.stations.length
            ? Object.values(gen.stationKms)
            : null);
      const hasIndexedStationKms = Array.isArray(generatedStationKmsInOrder);
      const stationKmFor = (i, st) => hasIndexedStationKms
        ? generatedStationKmsInOrder[i]
        : gen.stationKms?.[st.name];
      if (gen.stationKms || hasIndexedStationKms) {
        ...
        for (let i = 0; i < line.stations.length; i++) {
          ...
          const k = isLoopStart
            ? 0
            : (isLoopBack ? gen.totalKm : stationKmFor(i, st));
          if (k == null) continue;
