...
description: Use when adding, modifying, or reviewing React state, event handlers, Leaflet side-effects, gestures (swipe / drag-to-resize), or panel/sheet/modal interactions in `public/assets/app-core.jsx` and `public/assets/app-map.jsx`. The app uses React 18 UMD via Babel-standalone (no JSX build step) and Leaflet directly — review with that constraint in mind.
---
...
- **No build step for the React code.** `index.html` loads React/ReactDOM/Babel UMD and runs `<script type="text/babel">` blocks at runtime. Everything is `React.createElement(...)` — there is no JSX compilation. New components must follow the same calling convention.
- **Hooks are aliased**: `app-core.jsx` does `const { useState, useEffect, useMemo, useRef, useCallback } = React;` and `app-map.jsx` does the same with `M`-suffixed names to avoid TDZ issues across separately-loaded babel scripts. Keep that pattern.
...
## State graph (App in app-core.jsx)
...
1. **`mapRef` init effect** (`app-map.jsx`) — guarded by `if (leafletRef.current) return;` so it runs once. Do not add `[region]` to its deps; region changes are handled by the *fitBounds* effect.
...
- **Sheet drag-to-resize** (`app-map.jsx` TrainSheet): pointer down → set `dragRef.current.active`, `mousemove`/`touchmove` updates `userHeight` clamped to `[120px, 0.85 * mapH]`, persisted to `localStorage('relf.sheetHeight')`. Single click (no movement > 4 px) toggles collapse. Double-click resets to default.
- **Panel swipe-to-close** (`app-core.jsx` Panel): tracks first touch, closes when `dx < -60` and horizontal beats vertical motion.
